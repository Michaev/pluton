...
public class MultiSunspot {
	...
	public static MLRegression trainModel(
		...
		EncogUtility.trainToError(train, 0.0065);
	...
	public static File downloadSunSpotData() throws MalformedURLException {
		File rawFile = new File(MYDIR, "datapoints_LTC_USD.data");
	...
	public static TemporalMLDataSet predict(File rawFile, MLRegression model) {
		...
		while (csv.next()) {
			long timestamp = csv.getInt(0);
			double gainActual = csv.getDouble(1);
			double volume = csv.getDouble(2);
			...
			if( trainingData.getPoints().size()>=trainingData.getInputWindowSize() ) {
				...
				double gain = normSSN.deNormalize(modelOutput.getData(0));
				System.out.println(timestamp  + ":Predicted=" + gain + ",Actual=" + gainActual );
			...
			point.setSequence(sequenceNumber);
			point.setData(0, normSSN.normalize(gainActual) );
			point.setData(1, normDEV.normalize(volume) );
			trainingData.getPoints().add(point);
		}
		csv.close();
